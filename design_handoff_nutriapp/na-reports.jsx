/* ── NutriApp: Reports View ─────────────────────────────── */
const { useState: useStateRpt, useMemo: useMemoRpt } = React;

function ReportsView() {
  const [period, setPeriod] = useStateRpt('12w');
  ...
  const avgAdherence = Math.round(MOCK_ADHERENCE.flatMap(m => m.weeks).reduce((a, b) => a + b, 0) / MOCK_ADHERENCE.flatMap(m => m.weeks).length);
    ...
    <div className="na-reports-view">
      ...
      <div className="na-reports-content">
        ...
        <div className="na-stats-row">
          <StatCard icon={I.TrendDn} label="Peso perdido" value={`${lost} kg`} sub={`${startW} → ${endW} kg`} color="#E05555"/>
          <StatCard icon={I.Activity} label="IMC actual" value={bmiCurrent} sub={bmiCurrent < 25 ? "Normal" : "Sobrepeso"} color="#5B8DEF"/>
          <StatCard icon={I.Target} label="Adherencia" value={`${avgAdherence}%`} sub="Promedio 12 semanas" color="var(--accent-green)"/>
          <StatCard icon={I.Calendar} label="Duración" value="12 sem" sub={`Desde ${p.startDate.split('-').reverse().join('/')}`} color="#7E57C2"/>
        </div>
          ...
          <div className="na-chart-card wide">
            <div className="na-chart-header">
              ...
              <div className="na-chart-legend">
                <span className="na-legend-dot" style={{background:'var(--accent-green)'}}>Peso real</span>
                <span className="na-legend-dot" style={{background:'var(--accent-coral)'}}>Meta</span>
              </div>
            </div>
            <WeightChart data={w}/>
          </div>
...
function MacroDonut({ data }) {
  ...
  const total = data.proteinas + data.carbohidratos + data.grasas;
  const segments = [
              ...
              <tr key={f.key}>
                ...
                <td>
                  <span className="na-meas-diff" style={{ color: diff < 0 ? 'var(--accent-green)' : '#E05555' }}>
                    {diff > 0 ? '+' : ''}{diff} cm
                    {diff < 0 ? <I.TrendDn size={12}/> : <I.TrendUp size={12}/>}
                  </span>
                </td>
              </tr>
            );
